...
class AssetController extends Controller
{
    ...
    public function store(StoreAssetRequest $request)
    {
        ...
        foreach ($request->input('soft_beckup', []) as $file) {
            $asset->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('soft_beckup');
        }
    ...
    public function update(UpdateAssetRequest $request, Asset $asset)
    {
        ...
        if (count($asset->soft_beckup) > 0) {
            foreach ($asset->soft_beckup as $media) {
                if (! in_array($media->file_name, $request->input('soft_beckup', []))) {
                    $media->delete();
                }
            }
        }
        $media = $asset->soft_beckup->pluck('file_name')->toArray();
        foreach ($request->input('soft_beckup', []) as $file) {
