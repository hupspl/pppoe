...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="ip">{{ trans('cruds.asset.fields.ip') }}</label>
                <input class="form-control {{ $errors->has('ip') ? 'is-invalid' : '' }}" type="text" name="ip" id="ip" value="{{ old('ip', $asset->ip) }}">
                @if($errors->has('ip'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ip') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.ip_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="soft_beckup">{{ trans('cruds.asset.fields.soft_beckup') }}</label>
                <div class="needsclick dropzone {{ $errors->has('soft_beckup') ? 'is-invalid' : '' }}" id="soft_beckup-dropzone">
                </div>
                @if($errors->has('soft_beckup'))
                    <div class="invalid-feedback">
                        {{ $errors->first('soft_beckup') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.soft_beckup_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="date_service">{{ trans('cruds.asset.fields.date_service') }}</label>
                <input class="form-control datetime {{ $errors->has('date_service') ? 'is-invalid' : '' }}" type="text" name="date_service" id="date_service" value="{{ old('date_service', $asset->date_service) }}">
                @if($errors->has('date_service'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date_service') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.date_service_helper') }}</span>
            </div>
            <div class="form-group">
...
@section('scripts')
<script>
    var uploadedSoftBeckupMap = {}
Dropzone.options.softBeckupDropzone = {
    ...
    success: function (file, response) {
      $('form').append('<input type="hidden" name="soft_beckup[]" value="' + response.name + '">')
      uploadedSoftBeckupMap[file.name] = response.name
    },
    removedfile: function (file) {
      ...
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedSoftBeckupMap[file.name]
      }
      $('form').find('input[name="soft_beckup[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($asset) && $asset->soft_beckup)
          var files =
            {!! json_encode($asset->soft_beckup) !!}
              for (var i in files) {
              var file = files[i]
              this.options.addedfile.call(this, file)
              file.previewElement.classList.add('dz-complete')
              $('form').append('<input type="hidden" name="soft_beckup[]" value="' + file.file_name + '">')
            }
@endif
    },
